...
export default function TicketGenerator({ team }: TicketGeneratorProps) {
  ...
  const [ticketType, setTicketType] = useState<'attendance' | 'tv' | null>(
    null
  );
  ...
  // 오늘 날짜에 이미 티켓이 있는지 확인
  const today = new Date().toISOString().split('T')[0];
  const tickets = JSON.parse(localStorage.getItem('fanTickets') || '[]');
  const hasTodayTicket = tickets.some(
    (t: any) => t.team === team.id && t.date === today
  );
  ...
  const generateTicket = (type: 'attendance' | 'tv') => {
    ...
    setTimeout(() => {
      ...
      // Add to calendar
      const calendarEntries = JSON.parse(
        localStorage.getItem(`fanCalendar_${team.id}`) || '[]'
      );
      const calendarEntry = {
        id: Date.now() + 1,
        date: new Date().toISOString().split('T')[0],
        ...
        content: `${currentGame.stadium}에서 경기 ${
          type === 'attendance' ? '직관' : 'TV 시청'
        }`,
        emotion: 'excited',
  ...
  const getOpponentInfo = (opponentId: string) => {
    ...
    return (
      teams[opponentId as keyof typeof teams] || { name: '상대팀', logo: '⚾' }
    );
  };
    ...
    <>
        ...
        <CardContent className='p-4 text-center'>
          ...
          <div className='space-y-2'>
            <Button
              ...
              disabled={isGenerating || hasTodayTicket}
            >
      ...
      <Dialog open={showTicket} onOpenChange={setShowTicket}>
        <DialogContent className='max-w-sm'>
          <DialogHeader>
            <DialogTitle>
              {ticketType === 'attendance' ? '직관 티켓' : 'TV 시청 티켓'} 발급
              완료!
            </DialogTitle>
          </DialogHeader>
